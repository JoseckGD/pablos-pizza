...
export const ModalDeleteUser = ({ openModal, toggle, dataUser }) => {
  if (!openModal) return;

  const { data, isLoading, error, fetchData } = useFetch(
    `http://localhost:3000/api/usuarios/deleteUser/${dataUser?.id}`,
  ...
  return (
    ...
    >
      <div className="bg-white w-1/2 p-5 rounded-lg flex  gap-4 flex-col">
        <TitlePage title="Eliminar un empleado" />

        <p className="text-base text-center">
          Estas seguro de eliminar el empleado {dataUser?.nombre}?
        </p>
